...
contract ComptrollerG7 is ComptrollerV5Storage, ComptrollerInterface, ComptrollerErrorReporter, ExponentialNoError {
  ...
  function whitelistBorrowerAdd(address borrower) public override returns (uint256) {
    if (msg.sender != admin) {
  ...
  function setBorrowerLimits(address borrower, uint256 _borrowLimit) public override returns (uint256) {
  ...
  function getBorrowerLimits(address borrower) public view override returns (uint256) {
  ...
  function borrowAllowed(
    ...
    uint256 newBorrows = getNotionalBorrowsInternal(borrower) + borrowAmount;
  ...
  {
    (Error err, uint256 liquidity, uint256 shortfall) = getHypotheticalAccountLiquidityInternal(account, CToken(address(0)), 0, 0);
  ...
  {
    (Error err, uint256 liquidity, uint256 shortfall) = getHypotheticalAccountLiquidityInternal(account, CToken(cTokenModify), redeemTokens, borrowAmount);
    return (uint256(err), liquidity, shortfall);
  }
  ...
  function setCompSpeedInternal(CToken cToken, uint256 compSpeed) internal {
    ...
    if (currentCompSpeed != 0) {
      ...
      if (compSupplyState[address(cToken)].index == 0 && compSupplyState[address(cToken)].block == 0) {
        compSupplyState[address(cToken)] = CompMarketState({ index: compInitialIndex, block: safe32(getBlockNumber(), "block number exceeds 32 bits") });
      }

      if (compBorrowState[address(cToken)].index == 0 && compBorrowState[address(cToken)].block == 0) {
        compBorrowState[address(cToken)] = CompMarketState({ index: compInitialIndex, block: safe32(getBlockNumber(), "block number exceeds 32 bits") });
      }
    }
